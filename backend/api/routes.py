...
from ..analysis.yolo_extractor import extract_multi_keypoints_from_video
from ..analysis.popup_analyzer import analyze_popup_stages
...
async def analyze_video(
    ...
    try:
        if action == "popup":
            # 팝업: 여러 프레임 → 3단계 분석
            all_kps, all_frames = extract_multi_keypoints_from_video(str(video_path), num_samples=9)
            if not all_kps:
                raise ValueError("영상에서 사람을 감지하지 못했습니다.")
            popup_result = analyze_popup_stages(all_kps)

            # 오버레이: 중간 프레임 사용
            mid = len(all_frames) // 2
            keypoints_raw = all_kps[mid]
            frame = all_frames[mid]
