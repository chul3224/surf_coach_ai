...
def analyze_popup(kps: list[KeyPoint]) -> AnalysisResult:
    """
    체크포인트:
    - 무릎 굽힘 각도 (이상: 90~120도)
    - 손 위치 (가슴 옆에 있는지)
    ...
    """
    result = AnalysisResult(action="팝업(Pop-up)")
    scores = {}
    issues = []
...
def analyze_paddling(kps: list[KeyPoint]) -> AnalysisResult:
    """
    체크포인트:
    - 몸통 위치 (어깨가 보드 중심에 위치하는지 — 좌우 어깨 대칭)
    - 팔 스트로크 대칭 (왼팔꿈치 vs 오른팔꿈치 높이)
    - 머리 위치 (과도하게 들지 않는지)
    """
    ...
    if _visible(l_shoulder, r_shoulder):
        ...
        if tilt_ratio > 0.15:
            issues.append("몸이 한쪽으로 기울었습니다. 보드 중심에 눕는 자세를 유지하세요.")
            scores["몸통_대칭_점수"] = max(0, round(100 - tilt_ratio * 300))
        else:
            scores["몸통_대칭_점수"] = 100

    # 2) 팔꿈치 높이 대칭
    if _visible(l_elbow, r_elbow):
        elbow_y_diff = abs(l_elbow.y - r_elbow.y)
        scores["팔꿈치_높이_차이_px"] = round(elbow_y_diff, 1)
        if elbow_y_diff > 40:
            issues.append("좌우 패들링 스트로크가 불균형합니다. 양팔을 번갈아 균등하게 저으세요.")
            scores["패들링_대칭_점수"] = max(0, round(100 - elbow_y_diff * 0.8))
        else:
    ...
    if _visible(nose, l_shoulder, r_shoulder):
        ...
        if head_lift > 80:
            issues.append("머리를 너무 높이 들고 있습니다. 목에 힘을 빼고 자연스럽게 유지하세요.")
